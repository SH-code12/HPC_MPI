...
int main(int argc, char *argv[]) {
    ...
    MPI_Scatter(input, subSize, MPI_CHAR, 
               local_text,  subSize, MPI_CHAR, 
               0, MPI_COMM_WORLD);

    local_text[ subSize] = '\0';

    
    if (op == 1) {
        encrypt(local_text,  subSize);
    } else {
        decrypt(local_text, subSize);
    }

    
    MPI_Gather(local_text,  subSize, MPI_CHAR, 
              result_text,  subSize, MPI_CHAR, 
              0, MPI_COMM_WORLD);
